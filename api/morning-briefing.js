...
module.exports = async function handler(req, res) {
  try {
    ...
    const systemPrompt = `You are a personal assistant generating a morning briefing for Alexa to speak aloud.
Keep it concise, warm, and actionable — under 60 seconds of speaking time (roughly 150 words).
Structure: greet Mohammed, summarize today's key priorities based on active projects and recent daily notes,
highlight any deadlines or blockers, and end with an encouraging note.
Do NOT use markdown, bullet points, or special characters — this will be spoken aloud by Alexa.
Use natural conversational language.`;
 
    const briefing = await generateWithClaude(systemPrompt, notesContent);
